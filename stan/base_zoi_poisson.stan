...
parameters {
    ...
    real a_k; // avg k
    real a_b; // avg b
    vector a_pK; // avg p0, p1 (latent scale)
...
transformed parameters{
  ...
  for (n in 1:N_id) {
    ...
    p[n,1] = a_pK[1] + pop_v[pop_id[n],4];
    p[n,2] = a_pK[2] + pop_v[pop_id[n],5];
    p[n,3] = 0; // ref category
  }
...
model{
  ...
  // likelihood
        ...
        if (live_births[i] == 0) {
            target += log_sum_exp(
              categorical_logit_lpmf(1 | to_vector(p[pid[i],])),
               categorical_logit_lpmf(3 | to_vector(p[pid[i],])) + poisson_lpmf(live_births[i] | fert_cu)
              );
        }

        else if (live_births[i] == 1) {
            target += log_sum_exp(
              categorical_logit_lpmf(2 | to_vector(p[pid[i],])),
               categorical_logit_lpmf(3 | to_vector(p[pid[i],])) + poisson_lpmf(live_births[i] | fert_cu)
              );
        }

        else {
            target += categorical_logit_lpmf(3 | to_vector(p[pid[i],])) + poisson_lpmf(live_births[i] | fert_cu);
        }
    }
}
...
generated quantities {
   ...
   for (i in 1:N_obs) {
    ...
    fert_cu = pow(1 - exp(-k[pid[i]]*age[i]), b[pid[i]]) * alpha[pid[i]];
    k = categorical_logit_rng(to_vector(p[pid[i],]));
